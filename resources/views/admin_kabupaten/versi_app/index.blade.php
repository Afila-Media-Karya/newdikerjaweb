            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="text" id="version" class="form-control" name="version" placeholder="Masukkan versi">
                        <small class="text-danger version_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Status Versi</label>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Versi Aplikasi');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipart('/versi-aplikasi/store', 'Tambah', 'Versi Aplikasi','POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipart('/versi-aplikasi/update/' + uuid, 'Update','Versi Aplikasi', 'POST');
            }
        });

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/versi-aplikasi/show/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Versi Aplikasi', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/versi-aplikasi/delete/' + $(this).attr('data-uuid');
            ...
            let columnDefs = [{
                ...
                render: function(data, type, full, meta) {
                    return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="${full.nama_libur}" class="btn btn-danger button-delete btn-icon btn-sm"> 
                                <img src="{{ asset('admin/assets/media/icons/trash.svg')}}" alt="" srcset="">
                            </a>
                            `;
                    },
            }];
            control.initDatatable('/versi-aplikasi/datatable', columns, columnDefs);
        }
