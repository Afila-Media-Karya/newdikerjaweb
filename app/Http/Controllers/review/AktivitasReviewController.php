...
class AktivitasReviewController extends BaseController
{
    ...
    public function datatable()
    {
        ...
        // Ketika PLT, bawahan punya id_parent yang mengarah ke jabatan definitif,
        // bukan ke jabatan PLT. Cari jabatan definitif dengan id_master_jabatan yang sama.
        $id_jabatan_parent = $jabatan->id_jabatan;
        if ($jabatan->status == 'plt') {
            $definitif = DB::table('tb_jabatan')
                ->where('id_master_jabatan', $jabatan->id_master_jabatan)
                ->where('status', 'definitif')
                ->first();
            if ($definitif) {
                $id_jabatan_parent = $definitif->id;
            }
        }
