            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="number" class="form-control" name="tahun" placeholder="Contoh: 2025">
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function () { control.overlay_form('Tambah', 'Tanggal Ramadan'); });
        $(document).on('submit', ".form-data", function (e) {
            e.preventDefault();
            let type = $(this).attr('data-type');
            if (type == 'add') { control.submitFormMultipart('/ramadan/store', 'Tambah', 'Tanggal Ramadan', 'POST'); }
            else { let uuid = $("input[name='uuid']").val(); control.submitFormMultipart('/ramadan/update/' + uuid, 'Update', 'Tanggal Ramadan', 'POST'); }
        });
        $(document).on('click', '.button-update', function (e) { e.preventDefault(); control.overlay_form('Update', 'Tanggal Ramadan', '/ramadan/show/' + $(this).attr('data-uuid')); });
        $(document).on('click', '.button-delete', function (e) { e.preventDefault(); control.ajaxDelete('/ramadan/delete/' + $(this).attr('data-uuid'), $(this).attr('data-label')); });
        datatable = () => {
            let columns = [
            ...
            let columnDefs = [{
                ...
                <a href="javascript:;" data-uuid="${data}" data-label="${full.tahun}" class="btn btn-danger button-delete btn-icon btn-sm"><img src="{{ asset('admin/assets/media/icons/trash.svg')}}" alt=""></a>`
            }];
            control.initDatatable('/ramadan/datatable', columns, columnDefs);
        }
        $(function () { datatable(); })
    </script>
@endsection